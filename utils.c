...
create_group_table()
{
	...
	if (grpfilepath) {
		/* Use local passwd file in pwdir */
		if ((grpfile = fopen(grpfilepath, "r")) == NULL) {
			perror("Could not open group file");
			exit(1);
		}
		for (count = 0; fgetgrent(grpfile); count++);
		fclose(grpfile);
	} else {
	...
	if (grpfilepath) {
		...
		for (index = 0; (tmp = fgetgrent(grpfile)) && index < count; index++) {
...
create_passwd_table()
{
	...
	if (pwdfilepath) {
		/* Use local passwd file in pwdir */
		if ((pwfile = fopen(pwdfilepath, "r")) == NULL) {
			perror("Could not open passwd file");
			exit(1);
		}
		for (count = 0; fgetpwent(pwfile); count++);
		fclose(pwfile);
	} else {
	...
	if (pwdfilepath) {
		if ((pwfile = fopen(pwdfilepath, "r")) == 0) {
			perror("Could not open passwd file");
			exit(1);
		}
		for (index = 0; (tmp = fgetpwent(pwfile)) && index < count; index++) {
