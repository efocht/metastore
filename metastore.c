...
compare_fix(struct metaentry *real, struct metaentry *stored, int cmp)
{
	...
	if ((cmp & DIFF_MTIME) && S_ISLNK(real->mode)) {
		;
		/* EF: be silent about symlinks MTIME, this cannot be changed in Linux */
...
usage(const char *arg0, const char *message)
{
	...
	msg(MSG_CRITICAL, "Where ACTION is one of:\n"
	    ...
	    "  -d, --pwdir  <dir>\tDirectory for passwd file.\n"
	    );
...
main(int argc, char **argv, char **envp)
{
	...
	/* Paths for passwd and group files */
	if (pwdir) {
		pwdfilepath = malloc(strlen(pwdir)
				     + strlen("/passwd") + 2);
		strcpy(pwdfilepath, pwdir);
		strcat(pwdfilepath, "/passwd");
		grpfilepath = malloc(strlen(pwdir)
				     + strlen("/group") + 2);
		strcpy(grpfilepath, pwdir);
		strcat(grpfilepath, "/group");
	}
